...
Una mejora posible sería usar *lemmatizer* o *stemmer* para unificar features como "buen", "bueno", "buenos", "buena", etc. También se podrían introducir mejoras en el tokenizer respecto a features numéricos. En el caso de None se observan dos features favorables, "10" y "15", que podrían agruparse en un mismo token, ```$NUM$```, por ejemplo.

## Ejercicio 5 - Análisis de error

Para este ejercicio se añadió el script ```errors.py``` el cual selecciona las diez instancias peor clasificadas y las imprime por pantalla y las escribe en un archivo html.

El criterio elegido para determinar la magnitud del error fue la diferencia entre el puntaje asignado a la clase correcta y el asignado a la errónea.
...
A simple vista se puede observar que todos estos errores ocurrieron por asignarle la clase N (negativo) a tweets que tenían otro label. Esto podría indicar que el clasificador tiene ciertos features que favorecen excesivamente dicha clase.

Para corroborar esto, se seleccionó una instancia para hacer un análisis más detallado.
